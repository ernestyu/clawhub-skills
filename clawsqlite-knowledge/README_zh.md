...
- 优先尝试把 `clawsqlite>=1.0.0` 装到 Skill 运行时使用的默认 Python 环境；
...
> 仓库。唯一的代码来源就是 `pip install "clawsqlite>=1.0.0"`。

### 2.3 `clawsqlite` CLI 实际装在哪里？

取决于你的环境：

- 如果 `pip install clawsqlite>=1.0.0` 能在基础 venv 中成功：
...
- 用 jieba/启发式抽标签；
- 在 `clawsqlite>=1.0.0` 中，这套标签生成逻辑与查询侧关键词抽取共用
...
在知识库中检索，使用 `clawsqlite>=1.0.0` 提供的完整检索流水线：
...
## 7. 升级说明（clawsqlite>=1.0.0）

- 本 Skill 依赖 `clawsqlite>=1.0.0`，更新时会通过 `bootstrap_deps.py` 安装新的 PyPI 版本。
