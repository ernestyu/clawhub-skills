...
- during installation, it installs `clawsqlite>=0.1.4` (with a workspace-prefix fallback when the runtime env is not writable);
...
- First, it tries to install `clawsqlite>=0.1.4` into the default venv used for
  the Skill runtime.
